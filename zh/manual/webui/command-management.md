...
从 1.2.0 起，MaiBot 对插件命令提供**统一管理**：在 WebUI 中查看所有已注册的命令，并为命令配置**执行权限**（鉴权）。入口在「麦麦设置」页（侧边栏「麦麦配置编辑」分组 → 麦麦设置）顶部的 **命令管理** 编辑模式。
...
**验证**：在命令管理页搜索一个命令名，能搜到且鉴权标记正确，说明命令已注册。

**命令列表是空的？**

- 确认对应插件已安装并启用
- 插件未声明 Command 组件时不会出现在列表里

**保存放行规则失败？**

- 用户 ID 必须是 `platform:id` 格式
- 放行聊天只能从已存在的聊天流中选择
